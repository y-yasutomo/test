...
SELECT * FROM [mishina].[dbo].[practice] 
 ...
 SELECT [year]
         ,ROUND(AVG ([japanese]),1) AS j_ave
		 ,ROUND(AVG ([math]),1) AS m_ave
		 ,ROUND(AVG ([society]),1) AS so_ave
		 ,ROUND(AVG ([science]),1) AS s_ave
		 ,ROUND(AVG ([english]),1) AS e_ave
		 ,ROUND(AVG ([sum]),1) AS sum_ave		
  FROM [mishina].[dbo].[practice]
  GROUP BY [year]
  ; 
...
,(ROUND(([japanese] + [society] + [english])/3,1) - ROUND(([math] + [science])/2,1)) AS [gap]
FROM [mishina].[dbo].[practice]
ORDER BY (ROUND(([japanese] + [society] + [english])/3,1) - ROUND(([math] + [science])/2,1)) DESC
;
--答えが違う
